...
// Interface for SOCKS5 server access (not individual proxies)
...
export default function MyUserDetailPage() {
  ...
  // Current user's router and IP access
  const [currentUserRouterAccess, setCurrentUserRouterAccess] = useState<Set<string>>(new Set());
  const [currentUserIpAccess, setCurrentUserIpAccess] = useState<Set<string>>(new Set());

  // SOCKS5 Server Access states (servers, not individual proxies)
  ...
  // Fetch current user's router and IP access
  const fetchCurrentUserAccess = useCallback(async () => {
    if (!currentUser) return;

    // Get routers the current user has access to
    ...
    if (routerData) {
      setCurrentUserRouterAccess(new Set(routerData.map((r: { router_id: string }) => r.router_id)));
    }

    // Get IPs the current user has access to
    ...
    if (ipData) {
      setCurrentUserIpAccess(new Set(ipData.map((a: { ip_id: string }) => a.ip_id)));
    }

    // Get SOCKS5 servers the current user has access to
    const { data: socks5Data } = await supabase
      .from("user_socks5_server_access")
      .select("router_id")
      .eq("user_id", currentUser.id);

    if (socks5Data) {
      setCurrentUserSocks5ServerAccess(new Set(socks5Data.map((s: { router_id: string }) => s.router_id)));
    }
  ...
  const fetchUserData = useCallback(async () => {
    ...
    try {
      ...
      // Verify this user was created by the current user
      if (userData.created_by_user_id !== currentUser?.id) {
        toast.error("You can only manage users you created");
        router.push("/my-users");
        return;
      }
  ...
  }, [userId, supabase, router, currentUser, selectedRouterId]);

  // Fetch routers (only those the current user has access to)
  ...
  useEffect(() => {
    const checkAuth = async () => {
      ...
      if (!profile?.capabilities?.can_create_users && profile?.role !== "admin") {
  ...
  }, [currentUser, fetchRouters, fetchCurrentUserAccess, fetchUserData]);
  ...
  // Check if current user can manage a specific router
  const canManageRouter = (routerId: string): boolean => {
    return currentUserRouterAccess.has(routerId);
  };
  ...
  const handleGrantRouterAccess = async (routerId: string) => {
    if (!canManageRouter(routerId)) {
      toast.error("You don't have access to this router");
      return;
    }
  ...
  const handleRevokeRouterAccess = async (accessId: string, routerId: string) => {
    if (!canManageRouter(routerId)) {
      toast.error("You don't have permission to manage this router");
      return;
    }
  ...
  const handleGrantSocks5ServerAccess = async (routerId: string) => {
    if (!canGrantSocks5ServerAccess(routerId)) {
    ...
    try {
      ...
      if (error) {
        console.error("[SOCKS5 Server Access] Insert error:", error);
        if (error.code === "23505") {
          toast.error("User already has access to this SOCKS5 server");
        } else if (error.code === "42501") {
          toast.error("Permission denied - Run migration-v12 in Supabase");
        } else {
      ...
      console.error("[SOCKS5 Server Access] Error:", err);
      const error = err as Error;
      toast.error(`Failed to grant SOCKS5 server access: ${error.message}`);
    }
  };

  // Revoke SOCKS5 server access
  const handleRevokeSocks5ServerAccess = async (accessId: string, routerId: string) => {
    if (!canGrantSocks5ServerAccess(routerId)) {
      toast.error("You don't have permission to manage this SOCKS5 server");
      return;
    }
  ...
  const handleSaveUserInfo = async () => {
    ...
    try {
      ...
      const allowedCapabilities: UserCapabilities = {
        ...
        can_see_all_proxies: editCapabilities.can_see_all_proxies || false,
  ...
  // Get routers user doesn't have access to (that current user can grant)
  const availableRouters = allRouters.filter(
    r => !userRouterAccess.some(ur => ur.router_id === r.id) && canManageRouter(r.id)
  );
  ...
  const currentUserHasCap = (cap: keyof UserCapabilities) => {
    return currentUser?.capabilities?.[cap] === true;
  };
  ...
  return (
    ...
    >
      ...
      >
        <Button variant="outline" onClick={() => router.push("/my-users")} className="gap-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Grant capabilities to this user within your group
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  ...
                  <div className="flex items-center justify-between p-3 bg-secondary rounded-lg">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-medium text-sm">See All Proxies</p>
                        <p className="text-xs text-muted-foreground">Can see proxies from all users in the group</p>
                      </div>
                    </div>
                    <Checkbox
                      checked={editCapabilities.can_see_all_proxies || false}
                      onCheckedChange={(checked) =>
                        setEditCapabilities(prev => ({ ...prev, can_see_all_proxies: !!checked }))
                      }
                    />
                  </div>
                  ...
                  <div className="flex items-center justify-between p-3 bg-secondary rounded-lg">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-medium text-sm">See Group Peers</p>
                        <p className="text-xs text-muted-foreground">Can see peers from all users in the group</p>
                      </div>
                    </div>
                    <Checkbox
                      checked={editCapabilities.can_see_group_peers || false}
                      onCheckedChange={(checked) =>
                        setEditCapabilities(prev => ({ ...prev, can_see_group_peers: !!checked }))
                      }
                    />
                  </div>
            ...
            <Card>
              <CardHeader className="flex flex-row items-center justify-between">
                <div>
                  ...
                  <CardDescription>
                    Manage which routers this user can access (limited to routers you have access to)
                  </CardDescription>
                </div>
                ...
                {userRouterAccess.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    {userRouterAccess.map((access) => {
                      const canManage = canManageRouter(access.router_id);
                      return (
                        <div
                          key={access.id}
                          className={`flex items-center justify-between p-4 bg-secondary rounded-lg ${!canManage ? 'opacity-50' : ''}`}
                        >
                          <div className="flex items-center gap-3">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            {canManage ? (
                              <Button
                                variant="ghost"
                                size="icon"
                                onClick={() => handleRevokeRouterAccess(access.id, access.router_id)}
              ...
              <CardContent>
                {userRouterAccess.length === 0 ? (
                  ...
                  <div className="space-y-4">
                    ...
                    {/* IP list - only show IPs that current user can manage */}
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                      {(publicIpsByRouter[selectedRouterId] || [])
                        .filter(ip => canManageIp(ip.id))
                        .map(ip => {
                          ...
                          return (
                            <button
                              key={ip.id}
                              onClick={() => handleToggleIpAccess(ip.id, selectedRouterId, hasAccess)}
                    ...
                    <div className="pt-4 border-t border-border">
                      <div className="flex items-center justify-between text-sm">
                        ...
                        <Badge variant="outline" className="text-emerald-400">
                          {userIpAccess.filter(a => a.router_id === selectedRouterId && a.can_use && canManageIp(a.ip_id)).length}
                          {" / "}
            ...
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center justify-between">
                  ...
                  {/* Dropdown to grant server access - only show servers current user has access to */}
                  {allRouters.filter(r =>
                    canGrantSocks5ServerAccess(r.id) &&
                    !userSocks5ServerAccess.some(us => us.router_id === r.id)
                  ).length > 0 && (
                ...
                <CardDescription>
                  Grant access to SOCKS5 servers so this user can create their own proxies
                </CardDescription>
              </CardHeader>
              <CardContent>
                {userSocks5ServerAccess.length === 0 ? (
                  <div className="text-center py-8 text-muted-foreground">
                    <Network className="w-12 h-12 mx-auto mb-4 opacity-50" />
                    <p>No server access granted yet</p>
                    {currentUserSocks5ServerAccess.size === 0 && (
                      <p className="text-sm mt-2">You don&apos;t have access to any SOCKS5 servers to share</p>
                    )}
                  </div>
                ) : (
                  <div className="space-y-3">
                    {userSocks5ServerAccess.map((access) => {
                      const canManage = canGrantSocks5ServerAccess(access.router_id);
                      return (
                        <div
                          key={access.id}
                          className={`flex items-center justify-between p-3 bg-secondary/50 rounded-lg ${!canManage ? 'opacity-50' : ''}`}
                        >
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className="font-medium">
                                {access.routers?.name || "Unknown Server"}
                              </p>
                              <p className="text-sm text-muted-foreground font-mono">
                                {access.routers?.host || "-"}
                              </p>
                          ...
                          {canManage && (
                            <Button
                              variant="ghost"
                              size="icon"
                              onClick={() => handleRevokeSocks5ServerAccess(access.id, access.router_id)}
