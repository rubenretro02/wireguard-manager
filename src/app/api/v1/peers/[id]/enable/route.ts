...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const res = await setPeerEnabled(caller.admin, found.router, found.peer, true, {
    id: caller.userId,
    email: caller.email,
  });
  if ("error" in res) return apiError(res.error, res.status);
  return NextResponse.json({ peer: { ...found.peer, enabled: true } });
}
