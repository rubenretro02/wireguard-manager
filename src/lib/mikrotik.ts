...
class MikroTikRestClient {
  ...
  async executeCommand(path: string, params?: Record<string, unknown>): Promise<Record<string, unknown>[]> {
    // Convert /ip/firewall/nat/print to /ip/firewall/nat
    // Convert /ip/firewall/nat/add to PUT /ip/firewall/nat
    ...
    if (lastPart === "print") {
      ...
      const apiPath = path.replace(/\/add$/, "");
      const result = await this.request<Record<string, unknown>>("PUT", apiPath, params);
      return [result];
    } else {
...
class MikroTikClassicClient {
  ...
  async executeCommand(path: string, params?: Record<string, unknown>): Promise<Record<string, unknown>[]> {
    ...
    if (lastPart === "print") {
      // Get all items - /ip/firewall/nat/print -> /ip/firewall/nat
      const menuPath = path.replace(/\/print$/, "");
      const result = await api.menu(menuPath).getAll();
      return result as Record<string, unknown>[];
    } else if (lastPart === "add") {
      // Add new item - /ip/firewall/nat/add -> /ip/firewall/nat
      const menuPath = path.replace(/\/add$/, "");
      console.log(`[MikroTik Classic] Adding to ${menuPath}:`, params);
      const result = await api.menu(menuPath).add(params || {});
      console.log(`[MikroTik Classic] Add result:`, result);
      return [result as Record<string, unknown>];
    } else if (lastPart === "set" && params) {
      // Set/update item - /ip/firewall/nat/set
      const menuPath = path.replace(/\/set$/, "");
      const { id, ...updateParams } = params as { id?: string; [key: string]: unknown };
      if (id) {
        await api.menu(menuPath).where({ ".id": id }).update(updateParams);
      }
      return [];
    } else if (lastPart === "remove" && params) {
      // Remove item - /ip/firewall/nat/remove
      const menuPath = path.replace(/\/remove$/, "");
      const { id } = params as { id?: string };
      if (id) {
        await api.menu(menuPath).where({ ".id": id }).remove();
      }
      return [];
    } else {
      // Default: get all
      const result = await api.menu(path).getAll();
      return result as Record<string, unknown>[];
    }
  }
}
...
export class MikroTikClient {
  ...
  async executeCommand(path: string, params?: Record<string, unknown>): Promise<Record<string, unknown>[]> {
    if (this.restClient) return this.restClient.executeCommand(path, params);
    if (this.classicClient) return this.classicClient.executeCommand(path, params);
    throw new Error("No client configured");
  }
}
